...
class Database {
    ...
    public function sonuc($primary_id, $colums = "*", $table, $where)
    {
        if(is_null($where))
        {
            $query_string = 'SELECT '.$colums.' FROM '.$table;
            try {
                $query = $this->connect()->prepare($query_string);
                $query->execute();
                return $query->fetchAll(PDO::FETCH_ASSOC);
        ...
        {
            $query_string = 'SELECT '.$colums.' FROM '.$table.' where '.$where;
            try {
                $query = $this->connect()->prepare($query_string);
                $query->execute();
                return $query->fetchAll(PDO::FETCH_ASSOC);
     ...
     public function satir($colums = "*", $table, $where)
    {
        if(is_null($where))
        {
            return null;
